...
(use-package ocp-indent
  ...
  :load-path "/home/gaetan/.opam/4.07.0/share/emacs/site-lisp"
  :after tuareg
  :config
  (defmacro ocp-indent--with-input (&rest body)
    "If there are tabs, create a temporary buffer containing
current buffer's contents untabified and evaluate BODY there like
`progn'. See also `with-temp-buffer'."
    (declare (indent 0) (debug t))
    (let ((buf (make-symbol "buf")))
      `(if (not (save-excursion (goto-char (point-min)) (search-forward "\t" nil t)))
           (progn ,@body)
...
(defun debug-coq (coq)
  ...
  (let ((ocamldebug-command-name "../dev/ocamldebug-coq"))
    ...
    (if (string= (file-name-base coq) "coqchk")
        (debug-coq-sender "source checker_db")
