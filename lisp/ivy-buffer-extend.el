...
;; table, eg
;;
;; ivy-buffer-extend.el |Emacs-Lisp | ~/.emacs.d/lisp/ivy-buffer-extend.el |
  ...
  (cl-flet* ((calc-symbol-entry
    ...
    (let ((info_symbols
           (mapcar
            ...
            (cl-remove-if #'stringp format_spec))))
...
"
  (let ((cands_detail
    ...
    (cl-mapcar #'cons cands_detail candidates)))
...
(defmacro ivy--buffer-make-info-fn(sym real_buf_expr virt_buf_expr)
  `(cons ,sym
         (lambda (buffer)
           (let ((symfaces (cdr (assq ,sym ivy-buffer-format-faces))))
...
(defvar ivy--buffer-info-fns
  (list
   ...
   (ivy--buffer-make-info-fn
    'mode
    (with-current-buffer (get-buffer buffer) (format-mode-line mode-name))
    '"Virtual")
   ...
   (ivy--buffer-make-info-fn
    ...
    (shorten-directory (abbreviate-file-name
                        (or (with-current-buffer
                                (get-buffer buffer) default-directory) ""))
   ...
   (ivy--buffer-make-info-fn
    'process
    (if (get-buffer-process (get-buffer buffer)) "º" "")
    '"")

   (ivy--buffer-make-info-fn
    'file-name
    (file-name-nondirectory (or (buffer-file-name (get-buffer buffer)) ""))
    '"")
   ))
...
(defun ivy--buffer-display-list ()
  (let ((total_bufs (ivy--buffer-list "" ivy-use-virtual-buffers))
    ...
    (cl-delete-duplicates total_bufs :from-end t)
...
(defun ivy-extended-switch-buffer ()
